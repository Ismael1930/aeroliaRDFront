  ...
  useEffect(() => {
    ...
    if (vuelosOriginales.length > 0) {
      const vuelosOrdenados = [...vuelosOriginales].sort((a, b) => {
        switch (sortBy) {
          case 'precio-asc':
            // Ordenar por el precio mínimo de las clases disponibles
            const precioMinA = a.clasesDisponibles?.length > 0 
              ? Math.min(...a.clasesDisponibles.map(c => c.precio))
              : (a.precioBase || 0);
            const precioMinB = b.clasesDisponibles?.length > 0 
              ? Math.min(...b.clasesDisponibles.map(c => c.precio))
              : (b.precioBase || 0);
            return precioMinA - precioMinB;
          case 'precio-desc':
            // Ordenar por el precio mínimo de las clases disponibles
            const precioMinDescA = a.clasesDisponibles?.length > 0 
              ? Math.min(...a.clasesDisponibles.map(c => c.precio))
              : (a.precioBase || 0);
            const precioMinDescB = b.clasesDisponibles?.length > 0 
              ? Math.min(...b.clasesDisponibles.map(c => c.precio))
              : (b.precioBase || 0);
            return precioMinDescB - precioMinDescA;
          case 'hora-asc':
  ...
  return (
    <>
      {currentVuelos.map((vuelo) => {
        // Crear un card por cada clase disponible
        const clasesDisponibles = vuelo.clasesDisponibles || [];
        
        return clasesDisponibles.map((clase) => (
          <div className="js-accordion" key={`${vuelo.id}-${clase.clase}`}>
            <div className="py-20 px-30 bg-white rounded-4 base-tr mt-30">
                  ...
                  <div className="d-flex items-center h-full">
                    ...
                    <div>
                      <div className="text-right md:text-left mb-10">
                        ...
                        <div className="text-15 lh-16 text-blue-1 fw-500">
                          {clase.clase}
                        </div>
                        <div className="text-15 lh-16 text-light-1">
                          {clase.asientosDisponibles} asientos
                        </div>
                      </div>
                      <div className="mt-10">
                        <button
                      ...
                      <div className="accordion__button mt-10">
                        <button
                          ...
                          data-bs-target={`#vuelo-${vuelo.id}-${clase.clase}`}
                        >
              ...
              <div className="collapse" id={`vuelo-${vuelo.id}-${clase.clase}`}>
                {/* Detalles de IDA */}
                  ...
                  <div className="py-30 px-30 border-top-light">
                    ...
                    <div className="col-auto text-right md:text-left">
                      ...
                      <div className="text-14 mt-15 md:mt-5">
                        ...
                        <br />
                        {clase.asientosDisponibles} asientos disponibles
                      </div>
                    </div>
                  </div>
                </div>
